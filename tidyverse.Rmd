...
date: xx Nov 2019
author: Elizabeth Valles, Mike Jeziorski
output: 
      html_document:
         toc: TRUE
         toc_depth: 2
...
```
```{r, echo=FALSE}
htmltools::img(src = knitr::image_uri("images/RLadiesQro logo.png"), 
               alt = 'logo', 
...
![](images/Colin trophy.png) 
![](images/Colin ring.png) 
...
![](images/career-timeline-2.jpg)

***
### Not knowing your data can be dangerous  

Why don't we just use Excel for everything?  This is one reason:  

![](images/gene errors.png)
...
The quick analysis shows us that "week" is not week of the year, but number of weeks on the chart.  The "date.entered" column tells us when the song first appeared on the chart, but we do not yet have a variable that tells us the chart position for a specific week.  To make the data tidy, we want to do the following:  

- gather the data as we did above, now using "weeks" as the new variable
- remove the lines that have NA values for rank
- convert "wk1", "wk2", etc. to a numeric variable
- calculate a new variable, "date", that is date.entered + (# of weeks - 1) * 7
- select and reorder the columns
- sort the table to have the #1 songs at the top arranged by date
...
The tidyverse is constantly being improved.  The new version 1.0.0 of `tidyr` was released in September 2019 and has many new functions.  The `gather()` function has been replaced by `pivot_longer`, which allows us to replace three functions with one:

```{r echo=TRUE}
billboard %>% 
   pivot_longer(cols = starts_with("wk"),                            # pivot_longer gathers columns,
                names_to = "weeks",                                   
                names_prefix = "wk",                                 # gets rid of "wk" prefix,
                values_to = "rank",
                values_drop_na = TRUE) %>%                           # and filters out NAs
   mutate(weeks = as.numeric(weeks)) %>%                             # convert to numeric
   mutate(date = date.entered + 7 * (weeks - 1)) %>%                 # create a new variable
   select(date, rank, artist, track, weeks) %>%                      # reorder the columns
   arrange(rank, date)                                               # sort by rank and date
```

However, in this code we will use `gather`, which is documented on the `tidyr` cheat sheet.

***
We can already see some problems.  The #1 songs for the first two weeks of 2000 are missing.  Did we make an error or is the data missing from the dataset?  We will do some further analysis.

Once we are satisfied that the dataset looks tidy, we can edit the pipeline to save the dataframe as an object (NOT by overwriting the original data):
